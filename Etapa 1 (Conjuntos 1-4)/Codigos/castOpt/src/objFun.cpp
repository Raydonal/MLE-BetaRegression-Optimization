...
/* void STChebychev9(arma::vec& sum, arma::vec& x, arma::vec& N) */
/* /\*Calculate objective function value of x[].*\/ */
/* { */
/*   int i, j, m=60; */
/*   double d=72.661; */
...
/* void STChebychev17(arma::vec& sum, arma::vec& x, arma::vec& N) */
/* /\*Calculate objective function value of x[].*\/ */
/* { */
/*   int i, j,  m=100; */
/*   double d=10558.145; */
...
void reg_beta(arma::vec& sum, arma::vec& x, arma::vec& N, arma::mat& data)
{
  ...
  arma::vec eta = model_mat*x.rows(0,N[0]-2);
  ...
  sum[0] = n_rows*lgamma(phi) + arma::sum(temp);
}
