goTest <- function(par, fnName=c("Ackleys", "AluffiPentini",
  ...
  sum = 0
  do.call(fnName, list(sum = sum, x=as.double(par), N=length(par)))
  sum
}


goTestBetaReg <- function(par, test_matrix, fnName=c("reg_beta")){
  fnName <- match.arg(fnName)
  sum = 0
...
getInstance <- function(betaVector, types = c("runif", "rnorm"), phi, N, seed_X = NULL, seed_Y = NULL){
  # Convariate
  xSize <- length(betaVector)
  stopifnot(xSize == (length(types)+1))
  if (!is.null(seed_X)) set.seed(seed_X)

  xMat <- matrix(c(rep(1, N), runif((xSize - 1)*N)), nrow = N, ncol = xSize, byrow = FALSE)

  #xMat <- matrix(unlist(lapply(c("rep(1, N)", paste0(c("runif", "rnorm"), "(N)")), function(x){eval(parse(text = x))})), nrow = N, dimnames = list(1:N, paste0("X", 0:(xSize-1))))
